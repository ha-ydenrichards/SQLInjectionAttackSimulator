...
func _on_login_button_pressed() -> void:
	...
	u = u.replace(" ", "")
	# Store the password's text in p
	var p = password_input.text
	p = str(p)
	# Delete all whitespace
	p = p.replace(" ", "")
	# If the user attempts a valid SQL injection
	if u.contains("admin'OR") and u[9]==u[13] and p.contains("'OR'") and p[4]==p[8] or u.contains("manager'OR") and u[11]==u[15] or u == "admin'--" or u == "manager'--":
		win_sound.play()
		winning_text.show()
		random_text.hide()
