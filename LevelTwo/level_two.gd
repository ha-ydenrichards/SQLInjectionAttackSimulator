...
func _on_login_button_pressed() -> void:
	...
	p = p.replace(" ", "")
	p = str(p)
	# If the username entered is shorter than seven characters
	if u.length() < 7 and p.length() < 7:
		wrong_sound.play()
	# If the user attempts a valid SQL injection
	elif u.contains("'OR") and u.contains("=") and u[3]==u[7] and p.contains("'OR") and p.contains("=") and p[3]==p[7]:
		win_sound.play()
		winning_text.show()
		random_text.hide()
